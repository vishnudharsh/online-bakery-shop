...
function Shop() {
  ...
  const getAllProducts = async () => {
    const url = `http://localhost:9000/api/v1/product/product-list/${page}`;
    try {
      setLoading(true);
      const { data } = await axios.get(url);
      setLoading(false);
  ...
  const getTotal = async () => {
    const url = "http://localhost:9000/api/v1/product/product-count";
    try {
      const { data } = await axios.get(url);
      setTotal(data?.total);
    } catch (error) {
  ...
  const filterProduct = async () => {
    const url = "http://localhost:9000/api/v1/product/product-filters";
    try {
                              ...
                              <div className="product product-7 text-center">
                                <figure className="product-media">
                                  ...
                                  <div className="product-action">
                                    <a className="btn-product btn-cart" onClick={() => {
                                      setCart([...cart, p]);
